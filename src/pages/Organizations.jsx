...
/* ── Single dark field — top-level so no remount ───────────────── */
          ...
          {org.website && (
            <div style={{ display:'flex', gap:8, alignItems:'center' }}>
              ...
              <a href={org.website} target="_blank" rel="noopener noreferrer"
                style={{ fontSize:12, color:'var(--blue)', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap', maxWidth:220, textDecoration:'none' }}>
                {org.website}
              </a>
            </div>
          )}
...
function Row({ icon, text }) {
  return (
    <div style={{ display:'flex', gap:8, alignItems:'flex-start' }}>
      ...
      <span style={{ fontSize:12, color:'var(--ink2)', lineHeight:1.5 }}>{text}</span>
    </div>
  )
}
...
export default function Organizations() {
  ...
  /* ────────────────────────────────────────────────────────────
     ADD / EDIT FORM PAGE (dark theme matching screenshot)
  ──────────────────────────────────────────────────────────── */
  if(view==='add' || view==='edit') {
    const isEdit = view==='edit'
    return (
      <div style={{ minHeight:'100vh', background:'var(--paper2)', paddingTop:84, paddingBottom:60 }}>
        <div style={{ maxWidth:860, margin:'0 auto', padding:'0 32px' }}>

          {/* Back */}
          <button onClick={resetForm}
            style={{ display:'flex', alignItems:'center', gap:8, fontSize:13, fontWeight:600, color:'var(--ink2)', background:'none', border:'none', cursor:'pointer', marginBottom:28, padding:0 }}>
            ← Back to Organizations
          </button>

          {/* Title */}
          <h1 style={{ fontFamily:'Outfit,sans-serif', fontSize:36, fontWeight:900, color:'var(--ink)', letterSpacing:-.8, marginBottom:6 }}>
          ...
          {/* Form card */}
          <div style={{ background:'var(--paper)', border:`1px solid ${'var(--border)'}`, borderRadius:16, padding:28, marginBottom:24 }}>
            ...
            <div style={{ marginBottom:20 }}>
              ...
              {!logoPreview ? (
                <div onClick={()=>logoRef.current?.click()}
                  style={{ border:`2px dashed ${'var(--border2)'}`, borderRadius:12, padding:'36px 20px', textAlign:'center', cursor:'pointer', transition:'all .2s' }}
                ...
                <div style={{ display:'flex', gap:16, alignItems:'center', padding:'16px 18px', background:'var(--paper2)', borderRadius:12, border:`1px solid ${'var(--border)'}` }}>
                  <img src={logoPreview} style={{ width:60, height:60, objectFit:'cover', borderRadius:10, border:`2px solid ${'var(--blue)'}` }} alt=""/>
                  <div style={{ flex:1 }}>
            ...
            {/* Default ID card format info */}
            <div style={{ padding:'14px 16px', background:'var(--blue-s)', borderRadius:10, border:`1px solid rgba(35,82,255,.2)`, fontSize:13, color:'var(--blue)', fontWeight:600, display:'flex', gap:10, alignItems:'flex-start' }}>
  ...
  return (
    <div style={{ minHeight:'100vh', background:'var(--paper2)', paddingTop:84 }}>
      <div style={{ maxWidth:1200, margin:'0 auto', padding:'40px 32px' }}>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', marginBottom:32 }}>
          ...
          <button onClick={()=>{setForm(EMPTY_FORM);setErrors({});setLogoFile(null);setLogoPreview(null);setView('add')}}
            style={{ padding:'10px 20px', borderRadius:10, border:'none', background:'var(--blue)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:'inherit', display:'flex', alignItems:'center', gap:8 }}
          ...
          {ORG_TYPES.map(t => {
            ...
            return (
              <div key={t} onClick={()=>setFilterType(active?'All':t)}
                style={{ padding:'16px 12px', borderRadius:12, border:`1.5px solid ${active?'var(--blue)':'var(--border)'}`, background:active?'var(--blue-s)':'var(--paper)', cursor:'pointer', textAlign:'center', transition:'all .15s' }}>
                <div style={{ fontSize:22, marginBottom:6 }}>{ORG_ICONS[t]}</div>
                <div style={{ fontFamily:'Outfit,sans-serif', fontSize:22, fontWeight:900, color:active?'var(--blue)':'var(--ink)' }}>{count}</div>
                <div style={{ fontSize:11, fontWeight:600, color:'var(--ink2)', marginTop:3 }}>{t}</div>
              </div>
            )
          })}
        </div>
