...
const HANDLES = [
  {id:'nw',cx:0,cy:0},{id:'n',cx:.5,cy:0},{id:'ne',cx:1,cy:0},{id:'e',cx:1,cy:.5},
  {id:'se',cx:1,cy:1},{id:'s',cx:.5,cy:1},{id:'sw',cx:0,cy:1},{id:'w',cx:0,cy:.5},
]
...
/* ══════════════════════════════════════════════════════════════════
   ...
   FIX 1: startDrag no longer has `crop` in its useCallback deps.
           Instead, it reads crop from a ref (cropRef) that stays current.
           This prevents startDrag from being recreated on every crop change,
           which was causing rapid re-renders (→ blank screen) during drag.

   FIX 2: initCrop is called from the image's onLoad event, NOT from a
           setTimeout inside pickCustom. This eliminates the race condition
           where getBoundingClientRect() returned 0,0 on slow/mobile devices
           and triggered an infinite recursive setTimeout loop.

   FIX 3: CropModal is always rendered (not conditionally). The `open` prop
           controls visibility via the Modal component. This prevents the
           component from being destroyed/recreated on every photo change,
           which was resetting mid-crop state.

   FIX 4: Drag cleanup is guaranteed — the onUp handler always removes all
           listeners even if called multiple times (idempotent cleanup).
══════════════════════════════════════════════════════════════════ */
function CropModal({ open, imageUrl, onDone, onClose }) {
  const imgRef  = useRef(null)
  const dragRef = useRef(null)    // active drag metadata
  const cropRef = useRef(null)    // always-current copy of crop (avoids stale closure)
  ...
  // Keep cropRef in sync with crop state so drag closures always read fresh values
  useEffect(() => { cropRef.current = crop }, [crop])

  // Reset all state when modal opens with a new image
  useEffect(() => {
    if (!open) return
    setMode(null)
    setLoaded(false)
    setAutoResult(null)
    setImgSize({ w:1, h:1 })
    setCrop({ x:0, y:0, w:100, h:133 })
  }, [open, imageUrl])

  /* ── FIX 2: initCrop called after confirmed image load ── */
  const initCrop = useCallback(() => {
    if (!imgRef.current) return
    const r = imgRef.current.getBoundingClientRect()
    // Guard: if element has no size yet (e.g. display:none parent), bail
    ...
    setCrop({
      ...
      y: Math.round((ih - Math.min(ch, ih)) / 2),
      w: cw,
      h: Math.min(ch, ih),
    })
    setLoaded(true)
  ...
  /* ── Image onLoad: measure immediately after browser paints ── */
  const onImgLoad = useCallback(() => {
    if (mode === 'custom') {
      // Use rAF to ensure the img element has been laid out and painted
  ...
  const pickCustom = () => {
    setLoaded(false)
    setMode('custom')
    // The img element is already rendered. Re-measure after a rAF paint cycle.
    // This is safe even if the img loaded before we switched to custom mode.
    requestAnimationFrame(() => {
      requestAnimationFrame(initCrop)
    })
  }
  ...
  /* ── FIX 1: startDrag reads crop from cropRef, NOT from closure ──
     deps = [clamp] only, so startDrag is never recreated during a drag.
     This eliminates rapid re-renders that caused the blank screen.         */
  const startDrag = useCallback((e, type, hid = null) => {
    ...
    // Snapshot the crop at drag-start from the ref
    dragRef.current = { type, hid, startX:cx, startY:cy, startCrop:{ ...cropRef.current } }
    ...
    const onMove = (ev) => {
      ...
      setCrop(() => {
        ...
        const h = dragRef.current?.hid || ''
        let { x, y, w, hh } = { ...sc, hh:sc.h }
        ...
        if (h.includes('e') || h.includes('w')) hh = Math.round(w/ASPECT)
        else w = Math.round(hh*ASPECT)
        return clamp({ x, y, w, h:hh })
      })
    }

    /* ── FIX 4: Idempotent cleanup ── */
    const cleanup = () => {
  ...
  const handleApply = async () => {
    ...
    try {
      if (mode === 'auto') {
        ...
        const r = await applyCrop(imageUrl, crop, imgSize.w, imgSize.h)
        onDone(r)
        onClose()
      }
  ...
  return (
    <Modal open={open} onClose={onClose} title="📷 Crop Your Photo" width={640}>
      <style>{`
        ...
        .crop-img-box   { position:relative; background:#111; border-radius:10px; overflow:hidden;
                          height:280px; user-select:none; touch-action:none; }
        ...
        @media (max-width:540px) {
          ...
          .crop-img-box   { height:240px !important; }
          .crop-mode-grid { grid-template-columns:1fr 1fr; }
      ...
      <div className="crop-layout">
        ...
        <div className="crop-img-box">
          {/* FIX 3 (partial): image is always rendered when Modal is open,
              so getBoundingClientRect() always has a valid size.
              onImgLoad triggers initCrop via rAF to guarantee layout is done. */}
          <img
            ref={imgRef}
            src={imageUrl || ''}
            onLoad={onImgLoad}
            ...
            style={{
              ...
              opacity: mode==='auto' ? 0.35 : 1,
              transition:'opacity .3s',
            }}
            alt=""
          />

          {/* Auto mode: show cropped result overlay */}
          {mode==='auto' && (
          ...
          {/* Custom mode: dark overlay + draggable crop box */}
          {mode==='custom' && loaded && (
            <>
              {/* 4-piece dark overlay around crop box */}
              ...
              <div
                ...
                {[1/3, 2/3].map(f => (
                  <div key={`v${f}`} style={{ position:'absolute', left:`${f*100}%`, top:0, bottom:0, width:1, background:'rgba(255,255,255,.25)', pointerEvents:'none' }}/>
                ))}
                {[1/3, 2/3].map(f => (
                  <div key={`h${f}`} style={{ position:'absolute', top:`${f*100}%`, left:0, right:0, height:1, background:'rgba(255,255,255,.25)', pointerEvents:'none' }}/>
                ))}
          ...
          {!mode && (
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none' }}>
              <div style={{ background:'rgba(0,0,0,.55)', color:'#fff', fontSize:12, padding:'8px 16px', borderRadius:20 }}>
                Select a crop option ↓
              </div>
            </div>
          )}

          {/* Custom mode waiting for initCrop */}
          {mode==='custom' && !loaded && (
            <div style={{ position:'absolute', inset:0, display:'flex', alignItems:'center', justifyContent:'center', pointerEvents:'none' }}>
              <div style={{ background:'rgba(0,0,0,.55)', color:'#fff', fontSize:12, padding:'8px 16px', borderRadius:20 }}>
                ⏳ Preparing...
        ...
        <div className="crop-controls">
          <div className="crop-mode-grid">
            {[['auto','⚡','Auto','Smart center'], ['custom','✂️','Custom','Drag to select']].map(([m, icon, label, sub]) => (
              <div key={m}
                onClick={m==='auto' ? pickAuto : pickCustom}
          ...
          <div style={{ padding:'8px 10px', background:'var(--paper2)', borderRadius:'var(--r)', border:'1px solid var(--border)', fontSize:11, color:'var(--ink3)' }}>
            📐 Portrait 3:4 ratio
          </div>
...
/* ── RenderField — module-level (prevents focus-loss on re-render) ── */
    ...
    return (
      <div>
        ...
        <select value={val} onChange={e => { update('ClassN', e.target.value); update('Section', '') }}
          ...
          {orgClassesConfig.map(c => <option key={c.name} value={c.name}>{c.name}</option>)}
        </select>
        {errMsg}
      </div>
    )
...
export default function DetailsForm() {
  ...
  const handlePhoto = e => {
    ...
    r.onload = ev => {
      setPhotoRaw(ev.target.result)
      setShowCrop(true)
    }
    r.readAsDataURL(file)
    // Clear input AFTER reading (not before), to avoid triggering re-renders
    // that could interfere with the FileReader
  ...
  const handleConfirm = async () => {
    ...
    try {
      const sub = await createSubmission(
        { formConfigId:config.id, schoolName:config.school_name, role:config.role, ...formData },
        photoCropped
      )
  ...
  if (configLoad) return (
    <div style={{ display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', minHeight:'100vh', gap:16, background:'linear-gradient(135deg,#f0f4ff,#f5f6fc)' }}>
      <Spinner size={44}/>
      <p style={{ fontSize:14, color:'var(--ink3)', fontWeight:600 }}>Loading form...</p>
    </div>
  )
  if (notFound) return (
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(150deg,#f0f4ff 0%,#eef0f8 50%,#f5f6fc 100%)', padding:'24px 12px 48px' }}>
      <style>{`
        ...
        .df-photo-preview { display:flex; gap:16px; align-items:flex-start; }
            ...
            {hasPhoto && (
              <div style={{ marginTop:24, paddingTop:24, borderTop:'1px solid var(--border)' }}>
                ...
                {!photoCropped ? (
                  ...
                  <div className="df-photo-preview" style={{ padding:16, background:'linear-gradient(135deg,var(--teal-s),var(--blue-s))', borderRadius:12, border:'1.5px solid #00c48c' }}>
      ...
      {/* ── FIX 3: CropModal is ALWAYS rendered (not conditionally on photoRaw).
           The `open` prop + `imageUrl` prop control its visible state.
           This prevents the component from unmounting during crop operations,
           which was causing the blank screen on mobile.                        */}
      <CropModal
        open={showCrop && !!photoRaw}
        imageUrl={photoRaw || ''}
        onDone={setPhotoCropped}
