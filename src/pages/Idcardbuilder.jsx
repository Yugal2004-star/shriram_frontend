...
/* ═══════════════════════════════════════════════════════════
   CARD CANVAS  — drag fields AND photo directly on the card
═══════════════════════════════════════════════════════════ */
function CardCanvas({ config, sub, orgName, onMove, selected, onSelect }) {
  ...
  /* Universal mouse-down — works for both photo and text fields */
  const startDrag = (e, key, currentX, currentY) => {
    ...
    const onMouseMove = (ev) => {
      ...
      const maxX = key === '__photo__'
        ? CARD_W - (config.photoSize || 72)
        : CARD_W - 80
      const maxY = key === '__photo__'
        ? CARD_H - Math.round((config.photoSize || 72) * 4 / 3) - 30
        : CARD_H - 20
      const nx = Math.max(0, Math.min(maxX, dragRef.current.startX + dx))
      const ny = Math.max(0, Math.min(maxY, dragRef.current.startY + dy))
      onMove(key, Math.round(nx), Math.round(ny))
    }
    const onMouseUp = () => {
  ...
  const isPhotoSelected = selected === '__photo__'

  return (
    <div ref={canvasRef}
      style={{ position:'relative', width:CARD_W, height:CARD_H, background:'#fff', borderRadius:16,
      ...
      {/* Header */}
      <div style={{ background:headerBg, height:80, display:'flex', alignItems:'center',
        ...
        <div style={{ width:42, height:42, borderRadius:10, background:'rgba(255,255,255,.22)',
          display:'flex', alignItems:'center', justifyContent:'center',
          fontFamily:'Outfit,sans-serif', fontWeight:900, fontSize:15, color:'#fff', flexShrink:0 }}>
      ...
      {/* ── PHOTO — draggable ── */}
      <div
        onMouseDown={e => startDrag(e, '__photo__', px, py)}
        style={{
          position:'absolute', left:px, top:py, width:pw, height:ph,
          borderRadius:photoRadius,
          border:`2.5px solid ${isPhotoSelected ? config.c1 : config.c1}`,
          outline: isPhotoSelected ? `2px dashed ${config.c1}` : 'none',
          outlineOffset: 3,
          background:config.accent,
          display:'flex', alignItems:'center', justifyContent:'center',
          overflow:'hidden', cursor:'grab', zIndex:isPhotoSelected?50:15,
          boxShadow: isPhotoSelected ? `0 0 0 4px ${config.c1}33` : 'none',
          transition:'box-shadow .15s, outline .15s',
        }}>
        {sub?.photo_url
          ? <img src={sub.photo_url}
              style={{ width:'100%', height:'100%', objectFit:'cover', pointerEvents:'none' }} alt=""/>
        ...
        return (
          <div key={f.key}
            ...
            style={{
              ...
              cursor:'grab', zIndex:isSel ? 60 : 10,
            ...
            {isSel && (
              <div style={{ position:'absolute', top:-16, left:0, fontSize:9, color:config.c1,
                ...
                borderRadius:4, border:`1px solid ${config.c1}44`,
                boxShadow:'0 1px 4px rgba(0,0,0,.1)', pointerEvents:'none' }}>
                ↕↔ drag to move
              </div>
            )}
          </div>
      ...
      {config.showBarcode && (
        <div style={{ position:'absolute', bottom:0, left:0, right:0,
          background:`${config.c1}12`, padding:'8px 16px',
          ...
          <div style={{ fontSize:9, fontFamily:'JetBrains Mono,monospace',
            color:config.c1, fontWeight:600, opacity:.8 }}>
      ...
      {visibleFields.length === 0 && (
        <div style={{ position:'absolute', left:105, top:110, color:'#ddd',
...
export default function IDCardBuilder() {
  ...
  const orgName    = organizations.find(o => o.id === selectedOrg)?.name
                     || previewSub?.school_name || ''

  const upd = useCallback((key, val) =>
    setConfig(p => ({ ...p, [key]: val })), [])

  /* Called by CardCanvas on every mouse-move while dragging */
  const onMove = useCallback((key, x, y) => {
    if (key === '__photo__') {
      ...
      setConfig(p => ({
        ...p,
        fieldPositions: { ...(p.fieldPositions || {}), [key]: { x, y } }
      }))
    }
  }, [])
  ...
  const toggleField = (key) => {
    setConfig(p => {
      ...
      return { ...p, visibleFields: on
        ? p.visibleFields.filter(k => k !== key)
        : [...p.visibleFields, key] }
    })
  ...
  const handleSave = async () => {
    ...
    if (config.visibleFields.length === 0) {
      toast.error('Add at least one field to the card')
      return
    }
    setSaving(true)
    try {
  ...
  return (
    <div style={{ display:'flex', flexDirection:'column', height:'100vh',
      paddingTop:64, background:'var(--paper2)' }}>
      ...
      <div style={{ height:56, background:'var(--paper)', borderBottom:'1px solid var(--border)',
        ...
        <div style={{ display:'flex', alignItems:'center', gap:12, flex:1, minWidth:0 }}>
          ...
          {/* Template name — clearly visible input */}
        ...
        {/* Right: preview switcher + actions */}
        <div style={{ display:'flex', gap:10, alignItems:'center', flexShrink:0 }}>
          {approved.length > 1 && (
            <div style={{ display:'flex', alignItems:'center', gap:6 }}>
              <span style={{ fontSize:11, color:'var(--ink3)', whiteSpace:'nowrap' }}>Preview as:</span>
          ...
          <button onClick={handleSave} disabled={saving}
            style={{ padding:'7px 18px', borderRadius:8, border:'none',
              background: saving ? 'var(--border2)' : '#2352ff',
      ...
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'268px 1fr', overflow:'hidden' }}>
          ...
          <div style={{ display:'flex', borderBottom:'1px solid var(--border)', flexShrink:0 }}>
            {[['fields','📋 Fields'],['style','🎨 Style'],['settings','⚙ More']].map(([id,label]) => (
              <button key={id} onClick={() => setActiveTab(id)}
                style={{ flex:1, padding:'11px 4px', border:'none', fontSize:11, fontWeight:700,
          ...
          <div style={{ padding:'14px 12px', flex:1 }}>
            ...
            {activeTab === 'fields' && (
              <div>
                <p style={{ fontSize:11, color:'var(--ink3)', marginBottom:12, lineHeight:1.6 }}>
                  Click + to add a field. Click a field on the card to select it, then drag to reposition.
                </p>
                ...
                {config.visibleFields.length > 0 && (
                  <div style={{ marginBottom:12 }}>
                    ...
                    {ALL_FIELDS.filter(f => config.visibleFields.includes(f.key)).map(f => {
                      const isSel = selected === f.key
                      const pos   = config.fieldPositions?.[f.key]
                                  || DEFAULT_FIELD_POSITIONS[f.key] || { x:20, y:200 }
                      return (
                        <div key={f.key}
                          onClick={() => setSelected(isSel ? null : f.key)}
                          ...
                          <span style={{ flex:1, fontSize:12, fontWeight:600,
                            color:isSel?'var(--blue)':'var(--ink2)' }}>{f.label}</span>
                ...
                {ALL_FIELDS.filter(f => !config.visibleFields.includes(f.key)).map(f => (
                  <div key={f.key} onClick={() => toggleField(f.key)}
                    style={{ display:'flex', alignItems:'center', gap:8, padding:'7px 9px',
                      ...
                      background:'var(--paper)', cursor:'pointer', marginBottom:5,
                      transition:'all .15s' }}
            ...
            {activeTab === 'style' && (
              <div>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Organization</div>
                  <select value={selectedOrg} onChange={e => setSelectedOrg(e.target.value)}
                    style={{ width:'100%', padding:'8px 10px', borderRadius:8,
                      border:'1.5px solid var(--border)', background:'var(--paper)',
                      color:'var(--ink)', fontSize:12, fontFamily:'inherit', cursor:'pointer', outline:'none' }}>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Colors</div>
                  {[['Primary',config.c1,v=>upd('c1',v)],
                    ['Secondary',config.c2,v=>upd('c2',v)],
                    ['Accent BG',config.accent,v=>upd('accent',v)]].map(([l,v,fn]) => (
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:8 }}>
                        <input type="color" value={v} onChange={e => fn(e.target.value)}
                          style={{ width:26, height:26, borderRadius:6,
                            border:'1.5px solid var(--border)', padding:2, cursor:'pointer' }}/>
                        <span style={{ fontSize:10, fontFamily:'JetBrains Mono,monospace',
                          color:'var(--ink3)' }}>{v}</span>
                      </div>
                    </div>
                  ))}
                </div>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Header Style</div>
                  <div style={{ display:'flex', gap:8 }}>
                    {['gradient','solid'].map(v => (
                      <button key={v} onClick={() => upd('headerStyle',v)}
                        style={{ flex:1, padding:'8px', borderRadius:8,
                          border:`1.5px solid ${config.headerStyle===v?'var(--blue)':'var(--border)'}`,
                          background:config.headerStyle===v?'var(--blue-s)':'transparent',
                          color:config.headerStyle===v?'var(--blue)':'var(--ink3)',
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Photo</div>

                  {/* Select photo on card */}
                  <div onClick={() => setSelected(selected==='__photo__' ? null : '__photo__')}
                    style={{ display:'flex', alignItems:'center', gap:10, padding:'8px 10px',
                      borderRadius:8,
                      border:`1.5px solid ${selected==='__photo__'?'var(--blue)':'var(--border)'}`,
                      background:selected==='__photo__'?'var(--blue-s)':'var(--paper2)',
                      cursor:'pointer', marginBottom:10, transition:'all .15s' }}>
                    <span style={{ fontSize:16 }}>🖼</span>
                    <span style={{ flex:1, fontSize:12, fontWeight:600,
                      color:selected==='__photo__'?'var(--blue)':'var(--ink2)' }}>Photo</span>
                    <span style={{ fontSize:10, color:'var(--ink3)' }}>
                      {config.photoSize||72}×{Math.round((config.photoSize||72)*4/3)}px
                    </span>
                    <span style={{ fontSize:10, fontWeight:700,
                      color:selected==='__photo__'?'var(--blue)':'var(--ink3)' }}>
                      {selected==='__photo__'?'✓ Selected':'Select'}
                    </span>
                  </div>
                    ...
                    {['square','rounded','circle'].map(v => (
                      <button key={v} onClick={() => upd('photoShape',v)}
                        style={{ flex:1, padding:'7px 4px', borderRadius:8,
                          border:`1.5px solid ${config.photoShape===v?'var(--blue)':'var(--border)'}`,
                          background:config.photoShape===v?'var(--blue-s)':'transparent',
                          color:config.photoShape===v?'var(--blue)':'var(--ink3)',
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Border</div>
                  <div style={{ display:'flex', gap:8 }}>
                    {['none','thin','thick'].map(v => (
                      <button key={v} onClick={() => upd('borderStyle',v)}
                        style={{ flex:1, padding:'8px', borderRadius:8,
                          border:`1.5px solid ${config.borderStyle===v?'var(--blue)':'var(--border)'}`,
                          background:config.borderStyle===v?'var(--blue-s)':'transparent',
                          color:config.borderStyle===v?'var(--blue)':'var(--ink3)',
                          fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit',
                          textTransform:'capitalize' }}>
                        {v}
                      </button>
            ...
            {activeTab === 'settings' && (
              <div>
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Logo Position</div>
                  <div style={{ display:'flex', gap:8 }}>
                    {['left','center'].map(v => (
                      <button key={v} onClick={() => upd('logoPosition',v)}
                        style={{ flex:1, padding:'8px', borderRadius:8,
                          border:`1.5px solid ${config.logoPosition===v?'var(--blue)':'var(--border)'}`,
                          background:config.logoPosition===v?'var(--blue-s)':'transparent',
                          color:config.logoPosition===v?'var(--blue)':'var(--ink3)',
                          fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:'inherit',
                          textTransform:'capitalize' }}>
                        {v}
                      </button>
                ...
                <div style={{ marginBottom:14 }}>
                  <div style={{ fontSize:10, fontWeight:700, color:'var(--ink3)',
                    textTransform:'uppercase', letterSpacing:.6, marginBottom:7 }}>Font Size</div>
                  <div style={{ display:'flex', gap:8 }}>
                    {[['sm','Small'],['md','Medium'],['lg','Large']].map(([v,l]) => (
                      <button key={v} onClick={() => upd('fontSize',v)}
                        style={{ flex:1, padding:'8px 4px', borderRadius:8,
                          border:`1.5px solid ${config.fontSize===v?'var(--blue)':'var(--border)'}`,
                          background:config.fontSize===v?'var(--blue-s)':'transparent',
                          color:config.fontSize===v?'var(--blue)':'var(--ink3)',
                          fontSize:11, fontWeight:700, cursor:'pointer', fontFamily:'inherit' }}>
                        {l}
                      </button>
                    ))}
                ...
                <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between',
                  ...
                  <div onClick={() => upd('showBarcode', !config.showBarcode)}
                    style={{ width:38, height:22, borderRadius:11,
                      background:config.showBarcode?'var(--blue)':'var(--border2)',
                      transition:'background .2s', cursor:'pointer', position:'relative' }}>
                    <div style={{ width:18, height:18, borderRadius:'50%', background:'#fff',
                      position:'absolute', top:2,
                      left:config.showBarcode?18:2, transition:'left .2s',
        ...
        <div style={{ overflowY:'auto', display:'flex', flexDirection:'column',
          ...
          gap:18, background:'var(--paper2)' }}
          onClick={() => setSelected(null)}>

          {/* Hint */}
          <div style={{ display:'flex', alignItems:'center', gap:12,
            background:'var(--paper)', borderRadius:10, padding:'8px 18px',
            border:'1px solid var(--border)', fontSize:12, flexWrap:'wrap',
            justifyContent:'center' }}>
            <span style={{ color:'var(--blue)', fontWeight:700 }}>●</span>
            <span style={{ color:'var(--ink3)' }}>Click photo or any field on the card to select</span>
            <span style={{ color:'var(--border)' }}>|</span>
            <span style={{ color:'var(--ink3)' }}>Then drag to reposition</span>
            <span style={{ color:'var(--border)' }}>|</span>
            <span style={{ color:'var(--ink3)' }}>Resize photo with the slider in Style tab</span>
          </div>
          ...
          {/* Selected info */}
          {selected && (
            <div style={{ padding:'9px 16px', background:'var(--blue-s)',
              borderRadius:8, border:'1px solid var(--blue-m)',
              fontSize:12, color:'var(--blue)', fontWeight:600 }}>
              {selected === '__photo__'
                ? `🖼 Photo selected — drag on card to move · use Size slider to resize`
                : `✦ ${ALL_FIELDS.find(f=>f.key===selected)?.label} selected — drag on card to reposition`
              }
            </div>
          )}

          {approved.length === 0 && (
            <div style={{ padding:'10px 16px', background:'var(--amber-s)',
              borderRadius:8, border:'1px solid #fcd34d',
              fontSize:12, color:'#92400e', fontWeight:600 }}>
              ⚠ No approved submissions yet — showing placeholder data
            </div>
          )}
        </div>
      </div>
